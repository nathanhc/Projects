...
bool CharacterMoveComponent::CheckFootCast(CollisionComponentPtr& outComp, Vector3& outPos)
{
	...
	startPos.z += mFootCastOffset;

	Vector3 endPos = mOwner.GetPosition();
	endPos.z -= mFootCastOffset;

	if (mOwner.GetGame().GetPhysWorld().SegmentCast(mOwner, startPos, endPos, outComp, outPos))
	{

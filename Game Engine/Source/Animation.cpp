...
{
	float timeForFrame = mLength / (mNumFrames - 1);
	int frameA = inTime / timeForFrame;
	int frameB = frameA + 1;
	float percent = ((inTime / timeForFrame) - frameA) / (frameB - frameA);
	for (int i = 0; i < mTracks.size(); i++)
	{
		if (i == 0)
		{
			...
			{
				outPoses.push_back(Interpolate(mTracks[i][frameA], mTracks[i][frameB], percent).ToMatrix());
			}
			continue;
		}
		if (mTracks[i].empty())
		{
			outPoses.push_back(Matrix4::Identity * outPoses[inSkeleton->GetBone(i).mParent]);
		}
		else 
		{
			outPoses.push_back(Interpolate(mTracks[i][frameA], mTracks[i][frameB], percent).ToMatrix() * outPoses[inSkeleton->GetBone(i).mParent]);
		}
	}
}

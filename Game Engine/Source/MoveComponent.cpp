...
void MoveComponent::Tick(float deltaTime)
{
	...
	{
		//mOwner.GetGame().GetRenderer().GetPShaders().find(P_Rad)->second->GetPostConstants().movementx = -inputVelocity.x / 100;
		mOwner.GetGame().GetRenderer().GetPostProcess()->GetPostConstants().movementx = -inputVelocity.x / 100;
	}
	else 
	{
		//mOwner.GetGame().GetRenderer().GetPShaders().find(P_Rad)->second->GetPostConstants().movementx = inputVelocity.x / 100;
	...
	{
		//mOwner.GetGame().GetRenderer().GetPShaders().find(P_Rad)->second->GetPostConstants().movementy = -inputVelocity.y / 100;
		mOwner.GetGame().GetRenderer().GetPostProcess()->GetPostConstants().movementy = -inputVelocity.y / 100;
	}
	else
	{
		//mOwner.GetGame().GetRenderer().GetPShaders().find(P_Rad)->second->GetPostConstants().movementy = inputVelocity.y / 100;
...
void MoveComponent::SetProperties(const rapidjson::Value& properties)
{
	...
	if (GetFloatFromJSON(properties, "horizontalSpeed", horizontalSpeed))
	{

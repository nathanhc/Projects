...
RenderTargetPtr GraphicsDriver::CreateRenderTargetTexture(int inWidth, int inHeight, ETextureFormat inTextureFormat)
{
	D3D11_TEXTURE2D_DESC desc;
	...
	ID3D11Texture2D *pTexture = NULL;
	HRESULT hr;
	hr = g_pd3dDevice->CreateTexture2D(&desc, NULL, &pTexture);
	DbgAssert(hr == S_OK, "Problem Creating Texture From Memory");
	...
	ID3D11RenderTargetView* toRend = nullptr;
	hr = g_pd3dDevice->CreateRenderTargetView(pTexture, &renderTargetViewDesc, &toRend);
	DbgAssert(hr == S_OK, "Failed to create a render target view from texture");
	...
	ID3D11ShaderResourceView* toRet = nullptr;
	hr = g_pd3dDevice->CreateShaderResourceView(pTexture, &shaderResourceViewDesc, &toRet);
	DbgAssert(hr == S_OK, "Failed to create a shader resource view from texture");

	mShaderResource = GraphicsTexturePtr(toRet, AutoReleaseD3D);
	resources[0] = mShaderResource;

	return RenderTargetPtr(toRend, AutoReleaseD3D);
}
...
DepthStencilPtr GraphicsDriver::CreateDepthStencil(int inWidth, int inHeight)
{
	...
	// Create the shader resource view.
	ID3D11ShaderResourceView* toRend = nullptr;
	hr = g_pd3dDevice->CreateShaderResourceView(depthStencilTexture, &shaderResourceViewDesc, &toRend);
	DbgAssert(hr == S_OK, "Failed to create a shader resource from depth");

	mDepthResource = GraphicsTexturePtr(toRend, AutoReleaseD3D);
	resources[1] = mDepthResource;
